...
上游响应
   ...
   │
   └─ SourceRecord：每个源记录的当前投影
       active / withdrawn + current_raw_item_id
             │
             └─ Document：解析后的版本
...
- API、导出和 `report.html` 默认只读取 active Document；指定单文档仍能查看历史状态。
- M2 只使用 active Document。supersede/withdraw 会使事件物化失效并触发一致性重建。
...
`sources.yaml` 是 endpoint 启停的唯一真相：从 YAML 移除的 endpoint 会在 DB 自动变为 disabled/paused，避免旧配置继续产生幽灵抓取。
...
ETag 只在 changes cursor 没有移动、请求 URL 仍相同时复用，避免把 query-bound validator 错用于新 cursor。
...
Worker 使用三个互相独立的 APScheduler job：

| Job | 默认间隔 | 内容 |
|---|---:|---|
| ingest | 60 秒 | fetch → normalize（默认 500）→ fulltext（默认 20） |
| classify | 30 秒 | rules 默认 500；hybrid 默认 25，分别限流 |
| event | 60 秒 | dedupe → cluster |

每个 job `max_instances=1`，但不同 job 可以并行。慢 LLM、模型超时或事件全局重算不会阻塞 endpoint 的每日/小时级采集。Normalize 对一批记录使用外层事务并以 savepoint 隔离坏记录；默认规则分类也按批提交，避免大批回填时逐条事务的开销。
...
INTEL_LLM_API_KEY=...
INTEL_LLM_MODEL=...
INTEL_NORMALIZE_BATCH_SIZE=500
INTEL_FULLTEXT_BATCH_SIZE=20
INTEL_RULE_CLASSIFICATION_BATCH_SIZE=500
...
- `/sources`：endpoint 状态和 state_version；
- `/documents`：默认 active，`include_inactive=true` 可看历史；
- `/stats`：document lifecycle 与 model run 状态分布；
- `/ops/self-check`：源健康、FAILED 数量、所有处理中阶段的过期租约、M2 积压、分类配置、重试和 24 小时 fallback。
