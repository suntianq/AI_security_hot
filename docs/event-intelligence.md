...
M2.0 的目标是把 current `Document` 转成可查询、可追溯、可增量重放的 `Event`，同时保持保守合并：宁可暂时拆成两个事件，也不把两个不同漏洞错误合并。该阶段不依赖 LLM，模型不可用不会阻塞采集和事件化。
...
`raw_items.stage` 只表达采集、解析和全文完成状态；只有 DONE 文档进入分类。M2 使用 `Document.dedupe_version` 与 `Document.cluster_version` 作为独立状态，并且只消费共享当前谓词命中的 Document：`source_status=active`，同时 `record_status` 不是 `rejected/withdrawn`。
...
- 正文更新会清空分类、去重和聚类版本；分类变化会清空聚类版本；source revision/withdraw/retire 或 NVD Rejected/Withdrawn 会使旧证据退出当前视图并触发事件重建。
- 去重主记录或算法版本变化会使受影响文档的聚类版本失效。
- 没有待处理文档时，两个阶段直接返回 `status=current`，不会扫描全文。只要出现过期文档，M2.0 会全局重算组件/事件以保证新旧证据能重新选主，但只写入变化记录；持久化候选索引和局部组件重算属于 M2.1 优化。
- dedupe 使用标量游标读取，只在内存保留标题、标识、正文指纹与正文长度；结果按 2,000 条分批比较和更新，不把完整 ORM Document 再装载一遍。模糊候选索引仍是本次全局重放范围内的内存结构。
- cluster 按重复组件流式生成证据成员，先写入事务级 PostgreSQL 临时表，再按事件指纹流式聚合，并以 1,000 个事件为一批 upsert。临时表在提交/回滚时自动删除，完整重建仍保持单事务、advisory lock 和确定性输出，同时 Python 内存不随 CVE 事件总数线性增长。
...
```bash
uv run alembic upgrade head  # 升级到当前 head（含双轴 lifecycle/endpoint retirement/LLM audit）
...
该快照只用于验证量级，数据会随采集继续增长。测试环境为 3.6 GiB 内存主机：

- 362,479 份当前文档，分类待处理、去重待处理和聚类待处理均为 0。
- 1,520 份近重复。
- 360,674 个当前事件，403,132 条当前证据成员关系；另保留 297 条 superseded 事件历史证据关系。
- 所有当前文档都至少关联一个事件，孤儿关系为 0。
- 去重约 2 分 40 秒、峰值约 1.71 GiB；聚类约 2 分 16 秒、峰值约 116 MiB。
- 强制聚类复跑约 1 分 45 秒，事件/关系新增、更新、删除全部为 0，峰值约 116 MiB。
